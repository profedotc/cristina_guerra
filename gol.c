...
static int count_neighbors(struct gol *gol, bool *position);
static bool get_cell(struct gol *gol, bool *position);
...
void gol_step(struct gol *gol)
{
	...
	for (int x = 0; x < TAM_X; x++) {
		for (int y = 0; y < TAM_Y; y++) {
			vecinas_vivas = count_neighbors(gol, &gol->worlds[gol->current_world][x][y]);
...
int count_neighbors(struct gol *gol, bool *position)
{
	int vecinas_vivas = 0;
	vecinas_vivas += get_cell(gol, (position - (TAM_Y - 1)));
	vecinas_vivas += get_cell(gol, (position - TAM_Y));
	vecinas_vivas += get_cell(gol, (position - (TAM_Y + 1)));
	vecinas_vivas += get_cell(gol, (position + 1));
	vecinas_vivas += get_cell(gol, (position - 1));
	vecinas_vivas += get_cell(gol, (position + (TAM_Y - 1)));
	vecinas_vivas += get_cell(gol, (position + TAM_Y));
	vecinas_vivas += get_cell(gol, (position + (TAM_Y + 1)));
...
bool get_cell(struct gol *gol, bool *position)
{
	bool status = *position;

	bool *start = &gol->worlds[gol->current_world][0][0];
	bool *end = &gol->worlds[gol->current_world][TAM_X - 1][TAM_Y - 1];
